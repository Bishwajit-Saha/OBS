...
public class Transfer extends HttpServlet{
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String account_no_src = request.getParameter("account_no_src");
		String account_no_dest = request.getParameter("account_no_dest");
	...
	if(status == 1){
		...
		try{
			...
			ps2.setString(1,account_no_src);
			...
			size = 0; double balance2 = 0;
			if(status != -40 && status != -60){

				ps2 = con.prepareStatement("select amount from users_account where account_no=?");
				ps2.setString(1,account_no_dest);
				rs = ps2.executeQuery();

				while(rs.next()){
					balance2 = rs.getDouble("amount");
				...
				if(status!= -70){
					...
					ps2.setString(2,account_no_src);
					status = ps2.executeUpdate();
					ps2 = con.prepareStatement("update users_account set amount =? where account_no=?");
					ps2.setDouble(1,balance2);
					ps2.setString(2,account_no_dest);
					status = ps2.executeUpdate();
				}
			}
		...
		if(status>0){
			...
			String res = "AMOUNT SUCCESSFULLY TRANSFERED! BALANCE IS ";
			...
			request.setAttribute("res","SOURCE ACCOUNT DOES NOT EXIST");
		}
		else if(status == -70){
			request.setAttribute("res","DESTINATION ACCOUNT DOES NOT EXIST");
		}
